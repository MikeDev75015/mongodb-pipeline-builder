 ...
 * count: { $sum: 1 }
 * }
 * OPTIONAL granularity Optional. A string that specifies the preferred number series to use to ensure that the calculated
...
export const GroupByAutoPayload = (
    ...
    const payload: BucketAutoStageInterface = {
        groupBy: groupBy,
        buckets: buckets,
        output: { "count": { $sum: 1 } }
    };
