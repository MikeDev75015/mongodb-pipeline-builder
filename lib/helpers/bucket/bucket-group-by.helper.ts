 ...
 * @param optional
 * OPTIONAL defaultId Optional. A literal that specifies the _id of an additional bucket that contains all documents
 *   whose groupBy expression result does not fall into a bucket specified by boundaries. If unspecified, each input
 *   document must resolve the groupBy expression to a value within one of the bucket ranges specified by boundaries or
 *   the operation throws an error. The default value must be less than the lowest boundaries value, or greater than or
 *   equal to the highest boundaries value. The default value can be of a different type than the entries in
 *   boundaries. OPTIONAL output Optional. A document that specifies the fields to include in the output documents in
 *   addition to the
